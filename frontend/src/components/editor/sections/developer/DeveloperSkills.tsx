...
export default function DeveloperSkills({ section, dispatch }: Props) {
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="flex flex-wrap justify-center gap-4 md:gap-6">
          {content.skills.map((skill) => (
            <div key={skill.id} className="group/skill relative">
              <InlineEditable
                ...
                className="text-sm text-brown/60 border border-brown/20 rounded-full px-6 py-2.5 inline-block"
              />
              <button
                onClick={() =>
          ...
          <button
            ...
            className="text-sm rounded-full border border-dashed border-dark-green/20 px-6 py-2.5 text-brown/40 hover:border-gold hover:text-gold transition-colors"
          >
            + Add Skill
          </button>
        </div>
